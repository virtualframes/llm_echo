...
class TestJulesAgent:
    ...
    def test_audit_creates_artifacts(self, agent, tmp_path):
        ...
        agent.provenance_logger.log_dir.mkdir(parents=True, exist_ok=True)
        agent.echo_stream_viz.output_dir.mkdir(parents=True, exist_ok=True)
        agent.heatmap_viz.output_dir.mkdir(parents=True, exist_ok=True)
