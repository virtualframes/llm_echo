...
class HeatmapVisualizer:
    ...
    def create(self, flagged_posts: List[Dict[str, Any]], subreddits: List[str]) -> str:
        ...
        if any(data[sr]['count'] > 0 for sr in subreddits):
            ...
            sns.heatmap(normalized_matrix, annot=matrix_array, fmt='.2f',
                       xticklabels=metrics, yticklabels=[f"r/{sr}" for sr in subreddits],
                       cmap='YlOrRd', cbar_kws={'label': 'Normalized Score'},
                       linewidths=0.5, ax=ax)
            
            ax.set_title('Flagged Content Heatmap by Subreddit', 
